...
public final class DiskLruCache implements Closeable {
	...
	DiskLruCache(FileSystem fileSystem, File directory, int appVersion,
			int valueCount, long maxSize, Executor executor) {
	 ...
	 */
	public static DiskLruCache create(FileSystem fileSystem, File directory,
			int appVersion, int valueCount, long maxSize) {
		if (maxSize <= 0) {
		...
		return new DiskLruCache(fileSystem, directory, appVersion, valueCount,
				maxSize, executor);
	}
	...
	public synchronized Snapshot get(String key) throws IOException {
		...
		redundantOpCount++;
		journalWriter.writeUtf8(READ).writeByte(' ').writeUtf8(key)
				.writeByte('\n');
		if (journalRebuildRequired()) {
	...
	private synchronized Editor edit(String key, long expectedSequenceNumber)
		...
		journalWriter.writeUtf8(DIRTY).writeByte(' ').writeUtf8(key)
				.writeByte('\n');
		journalWriter.flush();
	...
	private boolean removeEntry(Entry entry) throws IOException {
		if (entry.currentEditor != null) {
			entry.currentEditor.hasErrors = true; // Prevent the edit from
													// completing normally.
		}
		...
		redundantOpCount++;
		journalWriter.writeUtf8(REMOVE).writeByte(' ').writeUtf8(entry.key)
				.writeByte('\n');
		lruEntries.remove(entry.key);
	...
	public synchronized void close() throws IOException {
		...
		for (Entry entry : lruEntries.values().toArray(
				new Entry[lruEntries.size()])) {
			if (entry.currentEditor != null) {
	 ...
	 * doesn't throw {@code ConcurrentModificationException}, but if new entries
	 * are added while iterating, those new entries will not be returned by the
	 * iterator. If existing entries are removed during iteration, they will be
	...
	public synchronized Iterator<Snapshot> snapshots() throws IOException {
		...
		return new Iterator<Snapshot>() {
			 ...
			 * modification errors.
			 */
			final Iterator<Entry> delegate = new ArrayList<>(
					lruEntries.values()).iterator();
	...
	public final class Snapshot implements Closeable {
		...
		private Snapshot(String key, long sequenceNumber, Source[] sources,
				long[] lengths) {
			this.key = key;
