...
public final class FramedStream {
	 ...
	 * {@link FramedDataSource#read}), but not yet acknowledged by sending a
	...
	/**
	 * Headers sent in the stream reply. Null if reply is either not sent or not
	 * sent yet.
	 */
	...
	FramedStream(int id, FramedConnection connection, boolean outFinished,
			boolean inFinished, List<Header> requestHeaders) {
		if (connection == null)
	...
	public synchronized boolean isOpen() {
		...
		if ((source.finished || source.closed)
				&& (sink.finished || sink.closed) && responseHeaders != null) {
			return false;
		}
		return true;
	}
	...
	/**
	 * Returns the stream's response headers, blocking if necessary if they have
	 ...
	 *             if this stream was initiated by the peer and a {@link #reply}
	...
	private final class FramedDataSource implements Source {
		...
		public long read(Buffer sink, long byteCount) throws IOException {
			...
			synchronized (FramedStream.this) {
				...
				read = readBuffer.read(sink,
						Math.min(byteCount, readBuffer.size()));

				// Flow control: notify the peer that we're ready for more data!
				unacknowledgedBytesRead += read;
				if (unacknowledgedBytesRead >= connection.okHttpSettings
						.getInitialWindowSize(DEFAULT_INITIAL_WINDOW_SIZE) / 2) {
					connection.writeWindowUpdateLater(id,
							unacknowledgedBytesRead);
					unacknowledgedBytesRead = 0;
				}
			}

			// Update connection.unacknowledgedBytesRead outside the stream
			// lock.
			synchronized (connection) { // Multiple application threads may hit
										// this section.
				connection.unacknowledgedBytesRead += read;
				if (connection.unacknowledgedBytesRead >= connection.okHttpSettings
						.getInitialWindowSize(DEFAULT_INITIAL_WINDOW_SIZE) / 2) {
					connection.writeWindowUpdateLater(0,
							connection.unacknowledgedBytesRead);
		...
		private void waitUntilReadable() throws IOException {
			readTimeout.enter();
			try {
				while (readBuffer.size() == 0 && !finished && !closed
						&& errorCode == null) {
					waitForIo();
				}
			} finally {
	...
	private void cancelStreamIfNecessary() throws IOException {
		...
		boolean cancel;
		synchronized (this) {
			cancel = !source.finished && source.closed
		...
		private void emitDataFrame(boolean outFinished) throws IOException {
			...
			try {
				connection.writeData(id,
						outFinished && toWrite == sendBuffer.size(),
						sendBuffer, toWrite);
			} finally {
				writeTimeout.exitAndThrowIfTimedOut();
			}
